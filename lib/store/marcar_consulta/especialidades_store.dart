...
abstract class _EspecialidadesStore with Store {
  ...
  @observable
  Map<String, dynamic> mapSpecialty = <String, dynamic>{};

  @action
  Future<void> fetchSpecialty() async {
    try {
      _db.collection('especialidades').snapshots().listen((snapshot) {
        dataSpecialtys = [];
        mapSpecialty = <String, dynamic>{};
        int lengthSpecialty = snapshot.docs.length;

        for (int i = 0; i < lengthSpecialty; i++) {
          String specialtyCapitalize =
              UtilsString.capitalize(snapshot.docs[i].id);
          dataSpecialtys = List.from(dataSpecialtys..add(specialtyCapitalize));
          mapSpecialty[specialtyCapitalize] =
              snapshot.docs[i].get("lista_funcionarios");
        }
      });
    } catch (e) {
      if (kDebugMode) {
